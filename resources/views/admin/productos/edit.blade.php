                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.productos.update', $producto->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group mt-2">
                                ...
                                <input type="text" class="form-control" name="ml" id="ml" value="{{ $producto->ml }}" required>
                            </div>

                            <div class="form-group mt-2">
                                <label for="concentracion">Concentración:</label>
                                <select class="form-control" name="concentracion" id="concentracion">
                                    <option value="Eau de Toilette" {{ $producto->concentracion == 'Eau de Toilette' ? 'selected' : '' }}>Eau de Toilette</option>
                                    <option value="Eau de Parfum" {{ $producto->concentracion == 'Eau de Parfum' ? 'selected' : '' }}>Eau de Parfum</option>
                                    <option value="Parfum" {{ $producto->concentracion == 'Parfum' ? 'selected' : '' }}>Parfum</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group mt-2">
                                ...
                                <input type="radio" id="hombre" name="categoria" value="Hombre" {{ $producto->categoria == 'Hombre' ? 'checked' : '' }}>
                                <label for="hombre">Hombre</label><br>
                                <input type="radio" id="mujer" name="categoria" value="Mujer" {{ $producto->categoria == 'Mujer' ? 'checked' : '' }}>
